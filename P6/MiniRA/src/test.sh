...
cd tests                                                        
for file in *.spg; do                                            
     T=`basename $file | sed s/.spg//`;                         
     java -jar -Xss2048k pgi.jar < $file > $T.out;    
done;                                                         
cd ..                                                       
...
i=0
j=0                                                
for file in tests/*.spg                               
do                                                      
     i=$((i+1))
     T=`basename $file | sed s/.spg//`                
     java RegAlloc < $file > /tmp/$T.miniRA
     ...
     java -jar kgi.jar < /tmp/$T.miniRA  > /tmp/$T.out
